...
cbo_engenheiro_civil <- c("2142-05", "2142-10", "2142-15", "2142-20", "2142-25", "2142-30", "2142-35", "2142-40")
...
```
### Escrevendo os dados filtrados em arquivo para evitar está requisitando ao Google Cloud sempre que o script for executado
...
ggplot(tabela_medias_CBO) +
  ...
  geom_col() +
  labs(y = "CBO", x = "Salário Médio (R$)")
```
```{r}
tabela_211110 <- tabela_microdados_vinculos |>
  filter(
    cbo_2002 %in% c("252515", "252525", "211205", "211210")
  )
tabela_media_uf <- tabela_211110 |>
  group_by(sigla_uf) |>
  summarise(
    media_por_regiao = median(valor_remuneracao_media)
  )
...
library(ggplot2)
codigos_cbo <- c("252515", "252525", "211110", "211205", "211210", "411035", "211215", "131120")
tabela_medias_ano <- tabela_microdados_vinculos |>
  group_by(ano, cbo_2002) |>
  summarise(media_salario = mean(valor_remuneracao_media))

ggplot(tabela_medias_ano) +
  aes(x = ano, y = media_salario) +
  geom_col() +
  labs(y = "CBO", x = "Salário Médio (R$)")
```

### 1ª Questão: Quanto a média salarial variou com o tempo?
```{r echo=TRUE}
library(ggplot2)
...
### 2ª Questão: Quanto a média salarial varia regionalmente?
```{r echo=TRUE}
...
tabela_media_uf |>
  ...
  labs(y = "Unidade da Federação", x = "Média Salarial (R$)")
```
#### Será que há muitos salários discrepantes puxando muito a média pra cima? Vamos verificar o comportamento no caso da mediana:
```{r}
